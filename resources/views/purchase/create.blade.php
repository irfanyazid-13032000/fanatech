...
@section('title', 'Tambah Data Purchase')
...
@section('content')
    ...
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light"><a class="text-decoration-none text-muted fw-light"
                href="{{ route('purchase.index') }}">Data Purchase</a> /</span>
        Tambah Purchase
    </h4>
    <div class="row">
        <!-- Form controls -->
                    ...
                    <form action="{{route('purchase.store')}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="number" name="number"
                                value="{{"Purchase-" . date('Y-m-d') . "-" . $lastId->id + 1}}" readonly>
                            @error('number')
                                <p style="color: rgb(253, 21, 21)">{{ $message }}</p>
                            @enderror
                        </div>

                        <div id="div-purchase-table"></div>
...
<script>
    ...
    function loadTableAwalSale() {
        var routeUrl = "{{ route('purchase.table.awal',':i') }}";
    ...
    function addRowSale() {
        $('#add-row').on('click',function (params) {
            ...
            var routeUrl = "{{ route('purchase.table.tambahan',':i') }}";
            routeUrl = routeUrl.replace(':i', i);
